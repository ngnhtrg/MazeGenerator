...
def NewGame(window, seg_len, start_point):
    """
        START NEW GAME

    :window: tk.Tk()
    :seg_len: length of a side of unit cell
    :start_point: coordinates of the starting point of maze
    """
    global path
    global vertical
    global horizontal
    ...
    play.draw_maze(
        window, grid[0], grid[1], WIDTH, HEIGHT, seg_len, start_point)
...
def go(
    maze, direction, vertical_lines, horizontal_lines,
    ...
    if direction == "U":
        ...
        if next in neighbours:
            ...
            if horizontal_lines[index] == 0:
                x1 = x_start + half_of_seg + (cur % width) * segment_len
                x2 = x_start + half_of_seg + (next % width) * segment_len
                y1 = y_start + half_of_seg + int(cur / width) * segment_len
                y2 = y_start + half_of_seg + int(next / width) * segment_len
                if next not in path:
                    maze.create_line(
                    ...
                    maze.create_line(
                        [(x1, y1), (x2, y2)], width=2, fill='white')
                    path.pop()
                maze.grid()
...
# NEW GAME
tk.Button(
    window, text="New Game",  bg="black", fg="white",
    command=lambda: NewGame(maze, segment_length, starting_point)
    ).grid(row=2, column=0)

# SOLUTION
tk.Button(
    window, text="Solution",  bg="black", fg="white",
    command=lambda: play.get_solution(
        maze, SOLUTION, WIDTH, HEIGHT, segment_length, starting_point)
    ).grid(row=2, column=1)

# Go UP
tk.Button(
    window, text="U",  bg="black", fg="white",
    command=lambda: go(
        maze, "U", vertical, horizontal,
        WIDTH, HEIGHT, segment_length, starting_point)
    ).grid(row=4, column=4)

# Go LEFT
tk.Button(
    window, text="L",  bg="black", fg="white",
    command=lambda: go(
        maze, "L", vertical, horizontal,
        WIDTH, HEIGHT, segment_length, starting_point)
    ).grid(row=5, column=3)

# Go RIGHT
tk.Button(
    window, text="R",  bg="black", fg="white",
    command=lambda: go(
        maze, "R", vertical, horizontal,
        WIDTH, HEIGHT, segment_length, starting_point)
    ).grid(row=5, column=5)

# Go DOWN
tk.Button(
    window, text="D",  bg="black", fg="white",
    command=lambda: go(
        maze, "D", vertical, horizontal,
        WIDTH, HEIGHT, segment_length, starting_point)
    ).grid(row=6, column=4)
