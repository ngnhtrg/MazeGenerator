...
def NewGame(window):
    ...
    Maze_with_graphical_interface.draw_maze(window, grid[0], grid[1], WIDTH, HEIGHT)
    vertical = grid[0]
    horizontal = grid[1]
    SOLUTION = grid[2]
...
tk.Button(window, text="Solution",  bg="black", fg="white", command=lambda: Maze_with_graphical_interface.get_solution(window,vertical, horizontal, SOLUTION, WIDTH, HEIGHT)).grid(row=2, column=1)
